...
template <typename T> class CompressorTest : public ::testing::Test {};
...
TYPED_TEST_P(CompressorTest, RoundTrip) {
  auto generateTestInputs = []() {
    ...
    std::generate(binary_data.begin(), binary_data.end(),
                  [&]() { return static_cast<char>(dist_byte(gen)); });
  ...
  for (const auto &original_str : generateTestInputs()) {
    ...
    bool is_printable =
        std::all_of(original_str.begin(), original_str.end(),
                    [](unsigned char c) { return c >= 32 && c <= 126; });

    if (is_printable && original_str.length() < 300) {
      // std::cout << "[ TESTING ] " << escape_for_code(original_str) <<
      // std::endl;
    }
    ...
    for (int level = -12; level < 10; ++level) {
      ...
      SCOPED_TRACE("Input String: " +
                   (original_str.length() > 100 && !is_printable
                        ? "[Binary Data]"
                        : escape_for_code(original_str)));
      SCOPED_TRACE("Compression Level: " + std::to_string(level));

      std::vector<uint8_t> compressed_data(
          compressor.get_max_compressed_size(original_data.size()));
      std::vector<uint8_t> decompressed_data(original_data.size());

      size_t comp_scratch_sz =
          compressor.get_compression_scratch_size(original_data.size());
      std::vector<uint8_t> comp_scratch(comp_scratch_sz);

      size_t compressed_size = 0;
      try {
        compressed_size = compressor.compress(original_data, compressed_data,
                                              comp_scratch, level);
      ...
      size_t decomp_scratch_sz =
          compressor.get_decompression_scratch_size(original_data.size());
      std::vector<uint8_t> decomp_scratch(decomp_scratch_sz);

      size_t decompressed_size = 0;
      try {
        decompressed_size = compressor.decompress(
            compressed_data, decompressed_data, decomp_scratch);
      } catch (const std::exception &e) {
...
using CompressorTypes =
    ::testing::Types<FSECompressor, LZCompressor<LZMode::V2B>,
                     LZCompressor<LZMode::V3B>, FuzzyLZCompressor>;
