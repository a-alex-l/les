...
int main(int argc, char *argv[]) {
  ...
  options.add_options()
      ...
      ("c,compress", "Enable compression mode with optional level (1-9)",
  ...
  try {
    ...
    if (!result.count("input")) {
      std::cerr << "Error: Input file is required. Use -i <filename>."
                << std::endl;
    ...
    if (result.count("output")) {
      ...
      if (compress_mode) {
        ...
        if (input_file.length() > extension.length() &&
            input_file.substr(input_file.length() - extension.length()) ==
                extension) {
          output_file =
              input_file.substr(0, input_file.length() - extension.length());
        } else {
          std::cerr << "Error: Cannot determine output filename. Input does "
                       "not end with '.les'. Use -o."
                    << std::endl;
          return 1;
        }
      }
    ...
    if (compress_mode) {
      ...
      // If the user typed "-c 5", cxxopts sets level=9 (implicit) and puts "5"
      // in positional. We check if we have a positional number 1-9 and override
      // the level.
      ...
      if (level < 1 || level > 9) {
        std::cerr << "Error: Compression level must be between 1 and 9."
                  << std::endl;
        return 1;
      }
