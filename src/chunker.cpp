...
void Chunker::compress_file(const std::string &input_file,
  ...
  // Common scratch buffer, large enough for any compressor
  // FuzzyLZ/LZ3B usually needs largest scratch. ~1MB + input overhead.
  // We resize dynamically if needed.
  ...
  while (in) {
    ...
    const size_t types_to_check = 1 + (level >= 4) + (level >= 7) +
                                  (level == 9) * (candidates.size() - 3);
...
void Chunker::decompress_file(const std::string &input_file,
  ...
  while (in.peek() != EOF) {
    ...
    // Check scratch requirements based on type and output size
    switch (type) {
    case CompressorType::FSE:
    case CompressorType::DELTA_FSE: {
      FSECompressor c;
